...
import { handleLevelUp, Task, Character } from "@/components/dashboard/dashboardUtils";
...
export default function DashboardPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<"todos" | "habito" | "diaria" | "afazer">("todos");
  const [toast, setToast] = useState<{ msg: string; type: "xp" | "hp" | "lvl" | "dmg" } | null>(null);
          ...
          <div className="max-w-7xl mx-auto w-full px-4 md:px-6 grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <section className="xl:col-span-6 space-y-6">
              ...
              <div className="space-y-3">
                {tasks.filter(t => activeFilter === "todos" || t.type === activeFilter).length > 0 ? (
                  tasks
                    .filter(t => activeFilter === "todos" || t.type === activeFilter)
                    .map(task => <TaskCard key={task.id} task={task} onComplete={completeTask} />)
                ) : (
                  <div className="bg-[#13111e]/50 border-2 border-[#2a2540] border-dashed py-24 text-center">
                    <p className="text-[#6b6480] text-sm uppercase tracking-[0.3em]">Mural Vazio</p>
                  </div>
                )}
              </div>
            </section>
