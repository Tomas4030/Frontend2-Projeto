...
type Props = { activeFilter: string; setActiveFilter: (f: "todos"|"habito"|"diaria"|"afazer") => void };
      ...
      {(["todos", "habito", "diaria", "afazer"] as const).map(f => (
        <button
          ...
          className={`flex-1 py-3 text-xs uppercase tracking-widest font-bold transition-all flex items-center justify-center gap-2 ${
            activeFilter === f ? "bg-[#f5c542] text-black shadow-inner" : "text-[#6b6480] hover:text-[#f5c542] hover:bg-[#1a162e]"
          }`}
        >
          {f}
        </button>
