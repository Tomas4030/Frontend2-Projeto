...
export default function LiquidFluid({
  ...
  useEffect(() => {
    ...
    const createTarget = (w: number, h: number) =>
      new RenderTarget(gl, {
        ...
        type: floatType,
        format: gl.RGBA,
        internalFormat: isIOS
          ? (gl as any).RGBA16F
          : (gl as any).RGBA32F || gl.RGBA,
        minFilter: gl.LINEAR,
        magFilter: gl.LINEAR,
        depth: false,
      });
    ...
    const programs = {
      advection: new Program(gl, {
        vertex: baseVertex,
        fragment: advectionFrag,
        uniforms: {
          tVelocity: { value: null },
          tSource: { value: null },
          dt: { value: dt },
          dissipation: { value: 0.98 },
          fboSize: { value: new Vec2() },
        },
      }),
      splat: new Program(gl, {
        ...
        uniforms: {
          tVelocity: { value: null },
          force: { value: new Vec2() },
          center: { value: new Vec2() },
          radius: { value: cursorSize / 1000 },
          aspect: { value: 1.0 },
        },
      }),
    ...
    const handleResize = () => {
      ...
      targetsRef.current = {
        vel0: createTarget(sw, sh),
        vel1: createTarget(sw, sh),
        pressure0: createTarget(sw, sh),
        pressure1: createTarget(sw, sh),
  ...
  useAnimationFrame((t) => {
    ...
    // Interaction
    const isAuto =
      autoDemo && Date.now() - lastInputTime.current > autoResumeDelay;
    const currentPos = isAuto
      ? new Vec2(autoPos.current.x, autoPos.current.y)
      : mouse.current;
    const diff = new Vec2(
      currentPos.x - lastMouse.current.x,
      currentPos.y - lastMouse.current.y,
    );
    ...
    if (velocity.current.len() > 0.0001) {
      ...
      progs.splat.uniforms.force.value.set(
        velocity.current.x * force,
    ...
    for (let i = 0; i < iterationsPoisson; i++) {
      progs.poisson.uniforms.tPressure.value = (
        i === 0 ? targets.pressure0 : targets.pressure0
      ).texture;
